...
String uid=hs.getAttribute("uid").toString();
 post=request.getParameter("textdata");
...
<a href="http://localhost:8080/MaliciousSocialMedia/ImageToText/examples/browser/file.html"><button style="float: right;">Upload Image</button></a>
</br>
</br>
<form method="post">

<input type="text" class="form-control" name="post" placeholder="Enter what is in your mind" value="<%out.println(post); %>" required>
</br>
...
if(str1!=null){
	...
	%>	
		...
		alert("Your post has been block due to malicious content to unblock mail us at samikshatadas@gmail.com");
...
PreparedStatement prst=con.prepareStatement("insert into UserPost(post,PostDate,UserId,UserName,violance,Vulgur,offensive,hate,clike,dlike)values(?,?,?,?,?,?,?,?,?,?)");
...
while(rs.next()){
	...
	<br/>
		...
		<br/>
		<a href="Like.jsp?id=<%=rs.getString(1) %>" style="color:#ffffff;">Like(<%out.println(rs.getString(10)); %>)</a> | <a href="Dislike.jsp?id=<%=rs.getString(1)%>" style="color:#ffffff;">Dislike(<%out.println(rs.getString(11)); %>)</a>
	<hr>
	<% 
}
}catch(Exception e){
...
<h2 style="color:#ffffff;"><a href="ShowAllPostUser.jsp">Show All Post</a></h2>
</div>
